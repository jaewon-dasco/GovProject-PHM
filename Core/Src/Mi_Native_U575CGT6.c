 ...
 *
 * 포팅 체크포인트:
 *   - Flash: 1 MB / 페이지 8 KB / 듀얼 뱅크 (L4: 256 KB / 2 KB)
 *   - SRAM:  SRAM1 192 KB @ 0x20000000 (L4 L433: 32 KB)
 *   - PWR:   HAL_PWREx_EnterSTOPxMode (STOP0/1/2/3) — L4 의 HAL_PWR_EnterSTOPMode 와 다름
 *   - Flash erase: FLASH_EraseInitTypeDef 의 Banks 필드 필요 (듀얼 뱅크)
 *   - RCC:   APB1/APB2 prescaler 레지스터 비트 이름 동일 (RCC_CFGR_PPRE1/PPRE2)
 *
...
// STM32U575CGT6: 1 MB Flash @ 0x08000000-0x080FFFFF, 8 KB page, dual bank
#define NATIVE_FLASH_ADDRESS_START   ((uint32_t)0x08000000)
#define NATIVE_FLASH_ADDRESS_END     ((uint32_t)0x080FFFFF)
#define NATIVE_FLASH_SIZE            (NATIVE_FLASH_ADDRESS_END-NATIVE_FLASH_ADDRESS_START)
#define NATIVE_FLASH_PAGE_SIZE       FLASH_PAGE_SIZE  // STM32U5: 8 KB

// STM32U575: SRAM1 192 KB @ 0x20000000-0x2002FFFF
#define NATIVE_SRAM_ADDRESS_FIRST     0x20000000
#define NATIVE_SRAM_ADDRESS_LAST      0x2002FFFF
...
oResult_t Native_FlashWriteAddress(uint32_t Address, uint8_t *pData, uint32_t SizeOfData, uint8_t CrcCheck)
{
    ...
    // STM32U5: 128-bit (16 byte) QUADWORD 단위로 플래시 쓰기
    while (WriteOfSize < SizeOfData && result == RESULT_RUN) {
        uint32_t QuadBuffer[4] = {0xFFFFFFFFu, 0xFFFFFFFFu, 0xFFFFFFFFu, 0xFFFFFFFFu};

        Size = MATH_MIN(SizeOfData - WriteOfSize, 16);
        memcpy(QuadBuffer, WriteOfPointer, Size);    // 16바이트 데이터 복사 (부족분은 0xFF padding)

        // 플래시에 기록 — U5는 데이터 포인터 전달, 16-byte aligned 주소 필요
        if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_QUADWORD, Address, (uint32_t)(uintptr_t)QuadBuffer) == HAL_OK) {
            Address += 16;
            WriteOfPointer += Size;
            WriteOfSize += Size;
        } else {
	...
	if(CrcCheck)
	{
	    // Step 3: CRC 계산 및 기록 — U5 QUADWORD (16 byte) 필요
	    uint32_t CrcQuad[4] = {0xFFFFFFFFu, 0xFFFFFFFFu, 0xFFFFFFFFu, 0xFFFFFFFFu};
	    uint32_t CalcCRC = oMEM_CRC32(pData, SizeOfData, 0);
	    CrcQuad[0] = CalcCRC;

	    if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_QUADWORD, Address, (uint32_t)(uintptr_t)CrcQuad) != HAL_OK) {
            result = RESULT_ERROR;
            goto EXIT;
		}
	}
...
oResult_t Native_SleepMode(uint32_t Millisecond)
{
	...
	//Start sleep
    HAL_SuspendTick();
    // STM32U5: STOP0/1/2/3 — STOP1 권장 (RAM 보존, 깨우는 데 가장 균형)
